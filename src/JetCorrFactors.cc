//
// $Id: JetCorrFactors.cc,v 1.2.4.2 2009/03/24 13:27:04 rwolf Exp $
//
...
JetCorrFactors::JetCorrFactors(float l1, float l2, float l3, float l4, FlavourCorrections l5, FlavourCorrections l6, FlavourCorrections l7)
{
...
JetCorrFactors::corrStep(const std::string &step, const std::string &flavour) {
  ...
  if(invalidCorrect){
    throw cms::Exception("InvalidRequest") << "invalid jet correction level " << flavour << " requested" << std::endl;
  }
  if(invalidFlavour){
    throw cms::Exception("InvalidRequest") << "invalid flavour " << flavour << " requested for jet correction: " << step << std::endl;
  }
  return result;
}
